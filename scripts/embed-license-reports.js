...
async function main() {
  ...
  if (!skipCommands) {
    log("step-1", "Running `pnpm generate` at repository root...");
    run("pnpm generate", repoRoot);
  } else {
    log("step-1", "Skipping `pnpm generate` (flag --skip-commands)");
  }
  ...
  log(
    "step-3",
    "Embedding LICENSES_3RD_PARTY.txt into pages/licenses/index.vue...",
  );
  ...
  if (!licensesIndexPath) {
    throw new Error(
      "Could not find licenses index.vue page in either pages/licenses/index.vue or pages/licenes/index.vue",
    );
  }
  replacePreBlock(licensesIndexPath, escapedThirdParty);
  log(
    "step-3",
    `Updated <pre> block in ${path.relative(repoRoot, licensesIndexPath)}`,
  );

  // Step 4: Extract <main> from src-tauri/license.html and replace in pages/licenses/rust.vue
  log(
    "step-4",
    "Replacing <main> in pages/licenses/rust.vue with generated Rust licenses HTML...",
  );
  const generatedHtmlPath = path.resolve(srcTauriDir, "license.html");
  if (!existsSync(generatedHtmlPath)) {
    throw new Error(
      `Generated HTML not found: ${generatedHtmlPath}. Did step 2 succeed?`,
    );
  }
  ...
  if (!rustVuePath) {
    throw new Error(
      "Could not find Rust licenses page in either pages/licenses/rust.vue or pages/rust.vue",
    );
  }
  replaceMainInVue(rustVuePath, newMainHtml);
  log(
    "step-4",
    `Updated <main> block in ${path.relative(repoRoot, rustVuePath)}`,
  );
